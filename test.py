...
from models.modules import Generator, CustomUnet
from models.io.hyperparameters_tools import load_optimal_hyperparameters, load_pickle_file
from postprocessing.operations import compute_morphological_operations
from performance.metrics import dice_by_sample, dice, hausdorff, f1_m, precision_m, recall_m, obj_metric, aji_score
...
def main():
    ...
    if args.method == "GAN":
        if os.path.isfile(os.path.join(args.checkpoint_path, "G_losses_system.pickle")):
            G_losses_GAN = load_pickle_file(os.path.join(args.checkpoint_path, "G_losses_GAN.pickle"))
            start_idx = min(args.start_epoch_checkpoint, len(G_losses_GAN) - 1)
            best_idx = np.argmin(G_losses_GAN[start_idx:]) + start_idx
